...
const sendEmail = async (to, subject, html) => {
  try {
    const fromEmail = process.env.SENDER_EMAIL; // Required env var validated at startup
...
const emailTemplates = {
  ...
  registrationConfirmation: (loginUrl) => {
    return {
      subject: 'تم إنشاء حسابك في بيكابو ✅',
      html: `
        <!DOCTYPE html>
        <html dir="rtl" lang="ar">
        <head>
          ...
          <style>
            ...
            .greeting { font-size: 18px; color: #2C3E50; margin-bottom: 16px; }
            .content { background: #E8F8F5; border-radius: 16px; padding: 20px; margin: 20px 0; text-align: center; color: #2C3E50; }
            .btn { display: inline-block; background: #27AE60; color: white; padding: 16px 32px; border-radius: 50px; text-decoration: none; margin: 20px 0; font-weight: bold; }
            .footer { text-align: center; color: #7F8C8D; font-size: 14px; margin-top: 24px; padding-top: 20px; border-top: 1px solid #EAEDED; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="logo">🎈 بيكابو</div>
            <h1 class="header">✅ مرحباً بك في بيكابو</h1>
            <p class="greeting">أهلاً بك 👋</p>
            <div class="content">
              <p><strong>تم إنشاء حسابك بنجاح في بيكابو.</strong></p>
              <p>يمكنك الآن تسجيل الدخول وإكمال الحجوزات بكل سهولة.</p>
            </div>
            <p style="text-align: center;">
            ...
            <div class="footer">
              <p>فريق بيكابو 🎪</p>
              <p style="font-size: 12px; color: #ABB2B9;">الدعم: support@peekaboojor.com</p>
            </div>
          </div>
        </body>
        </html>
